...
**Adobe private source + public GitHub for clients:** read **[ARCHITECTURE.md](./ARCHITECTURE.md)** first.

This repo is the **public** entry point. **`flags-ios` stays on Adobe Git only.** Clients never clone Adobe; they resolve a **binary** from [GitHub Releases](https://github.com/pandey-vivek/flags-ios-spm/releases).

---

Legacy modes (for learning / local only):
...
## Mode A — Quick test (local path, both repos on disk)

**Do not use the default `Package.swift` on GitHub for this** — it points at `https://github.com/pandey-vivek/flags-ios.git`.
...
## Mode A2 — Remote SPM (what Xcode uses after you add the GitHub URL)

**Requirements:**

1. **`flags-ios`** is pushed to **`https://github.com/pandey-vivek/flags-ios`** (same account as `flags-ios-spm`).
2. Tag **`0.1.0`** exists on **both** repos (GitHub → Releases → Create release, tag `0.1.0`).
3. **`flags-ios-spm`** `Package.swift` on `main` uses the **Git URL** (not `path: ../flags-ios`).
...
**If resolution fails with “unstable-version” or “flags-ios”:** the published `Package.swift` still had `.package(path: "../flags-ios")` — push the fixed `Package.swift` and tag again.
